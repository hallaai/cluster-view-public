...
function calculateSilhouetteScore(node: number, clusterMembers: number[], allClusters: Cluster[], distanceMap: Map<number, Map<number, number>>): number {
    if (clusterMembers.length <= 1) {
        return 0; // Silhouette score is 0 for single-member clusters
    }

    // a: Mean intra-cluster distance
    let sumIntraDistance = 0;
    for (const member of clusterMembers) {
        sumIntraDistance += getDistance(node, member, distanceMap);
    }
    const a = sumIntraDistance / (clusterMembers.length - 1); // Divide by N-1 for within-cluster points

    // b: Mean nearest-cluster distance
    let minNearestClusterDistance = Infinity;

    for (const otherCluster of allClusters) {
        // Only consider other clusters that the node is NOT a part of
        if (!otherCluster.members.includes(node)) {
            let sumOtherClusterDistance = 0;
            for (const otherMember of otherCluster.members) {
                sumOtherClusterDistance += getDistance(node, otherMember, distanceMap);
            }
            const meanOtherClusterDistance = sumOtherClusterDistance / otherCluster.members.length;
            minNearestClusterDistance = Math.min(minNearestClusterDistance, meanOtherClusterDistance);
        }
    }

    const b = minNearestClusterDistance;

    // Silhouette Coefficient
    if (b === Infinity) { // Node is in a cluster with no other clusters nearby
        return 0; 
    }
...
export async function runClusteringAlgorithm(
  ...
  for (let k = params.minClusters; k <= params.maxClusters; k++) {
    ...
    allMetrics.push({ 
        ...
        silhouetteScore: 0 // Placeholder - calculate below if clusters are valid
    });

    // Calculate average Silhouette score for this k
    if (resultForK.clusters.length > 0) {
        let totalSilhouetteScore = 0;
        let nodeCountForSilhouette = 0;
        for (const cluster of resultForK.clusters) {
            for (const node of cluster.members) {
                totalSilhouetteScore += calculateSilhouetteScore(node, cluster.members, resultForK.clusters, distanceMap);
                nodeCountForSilhouette++;
            }
        }
        // Assign the calculated average Silhouette score to the corresponding metric entry
        const metricEntry = allMetrics.find(metric => metric.k === k);
        if (metricEntry && nodeCountForSilhouette > 0) metricEntry.silhouetteScore = totalSilhouetteScore / nodeCountForSilhouette;
    }
