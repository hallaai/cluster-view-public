...
import { Bar, BarChart, CartesianGrid, XAxis, YAxis, LabelList, ResponsiveContainer, Tooltip as RechartsTooltip, Line, LineChart as RechartsLineChart } from 'recharts';
import {
  ChartConfig,
  ChartContainer,
...
export function ClusterResultsDisplay({ results, isProcessing }: ClusterResultsDisplayProps) {
 ...
 silhouetteScore: parseFloat(metric.silhouetteScore?.toFixed(3) || '0'), // Add silhouette score
 chosen: metric.k === results.chosenK ? 'Chosen K' : 'Other K'
  ...
  return (
    <div className="w-full max-w-4xl mt-8 space-y-8">
      ...
      {/* Display Silhouette Score Chart */}
 {results.allMetrics && metricsChartData.length > 0 && (
        <Card className="shadow-lg">
          <CardHeader>
            <CardTitle className="font-headline text-2xl flex items-center">
            ...
            <CardDescription>
              This chart displays the Silhouette score for each tested number of clusters (k).
              Higher Silhouette scores indicate that objects are well-clustered, as they are more similar to their own cluster compared to other clusters.
              The Silhouette score ranges from -1 to +1, where a score close to +1 means the object is far away from the neighbor clusters, a score of 0 means the object is on or very close to the decision boundary between two neighbor clusters and negative values indicate that those objects might have been assigned to the wrong cluster.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {metricsChartData.length > 0 ? (
                <ChartContainer config={silhouetteChartConfig} className="min-h-[300px] w-full">
                <RechartsLineChart accessibilityLayer data={metricsChartData} margin={{ top: 20, right: 20, left: 0, bottom: 20 }}>
                    <CartesianGrid vertical={false} />
                    <XAxis 
                        dataKey="k" 
                        name="Number of Clusters (k)" 
 ...
 domain={[-0.1, 1.1]} // Set domain for better visualization
                        allowDecimals={true}
                    />
                    <RechartsTooltip 
                        cursor={{ fill: 'hsla(var(--muted), 0.5)' }} 
                    ...
                    />
                    <Line type="monotone" dataKey="silhouetteScore" stroke="var(--color-silhouetteScore)" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 6 }} />
                </RechartsLineChart>
                </Chart>
            ) : (
                <p className="text-muted-foreground p-4 text-center">No metric data available to display comparison chart. This might occur if no valid clusters were formed for any k value tested.</p>
